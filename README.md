...
第四五章使用机器语言描述，我理解为固定输入位数的hdl。如果前面的硬件输入都是8位或者16位，则可以用二进制01组成的8位或者16位表示输入。  
本文是16位输入，通过左边第一位区分指令类型，分为A指令、C指令。A指令表示常数；C指令表示控制，其中某几位表示不同的控制功能，comp,dest,jump。  
因为硬件是按照一定的规范设计的，所以可以通过16位的机器语言来操作硬件进行计算，存储，跳转。但是16位的机器语言由人工编写太困难了。加入了符号，使用符号来代替机器语言，就是汇编。  
要运行汇编语言，还是需要先把汇编转为机器语言的。

第六章主要实现汇编编译器：把汇编(asm)翻译成机器语言(hack)。汇编中涉及符号，预定义符号，标签符号，变量符号。**这里符号涉及到作用域，生命周期，内存管理。**

第七八章主要实现虚拟机：将虚拟语言(VM)翻译成汇编语言(asm)。虚拟语言中涉及参数，局部变量，静态变量，常数，this，临时变量等转换成汇编中对应的符号，包括堆栈，就是在内存中划分出不同作用的位置。

第九十十一章主要实现编译器：将高级语言(jack)翻译成虚拟语言(VM)。高级语言中有比较复杂的语法，数据和控制逻辑，利于人类理解和编写。
